...
// TODO: Почему-то при обычном полном обновлении каждый раз появляется один бесхозный сохраненный объект (утечка памяти)
...
class Importer
{
	...
	static public function UpdateNodePathInfo($node, $currentWay, $unsetSource = false)
	{
		...
		foreach ($keysArray as $key) {
			...
			if ($val instanceof Ref && $val->_OWNER) {
				...
				$val = $val->_DAT;
				self::UpdateNodePathInfo($val, $currentWay . self::WAY_SEPARATOR . $key, $unsetSource);
			}
		}
	}
	...
	static private function ImportDirectoryFromFileSystem(&$object, $wayFromRoot, $fullRealPath)
	{
		...
		$scriptFileName = realpath($fullRealPath . self::FILE_SEPARATOR . $lastDirName . '.php');
		if (file_exists($scriptFileName)) {
			...
			if (is_array($res)) $res = new Data($res);
		} else {
			$res = new Data();
		}
		...
		while (($childFileName = readdir($dh)) !== false) {
			$fullChildPath = $fullRealPath . self::FILE_SEPARATOR . $childFileName;
			if (is_dir($fullChildPath) && substr($childFileName, 0, 1) != ".") {
				if (!isset($object[$childFileName])) {
					$childWayFromRoot = $wayFromRoot . self::WAY_SEPARATOR . $childFileName;
					$child = null;
		...
		if ($scriptFileName !== false) {
			...
			$sourceInfo->fileNode[$scriptFileName] = new Ref($res, false);
	...
	static private function FS($way)
	{
		...
		$stub = new Data(['wayFromRoot' => $wayFromRoot]);
	...
	static private function FS_OWN($path, $wayOffset = null)
	{
		...
		if (!file_exists($fullRealPath)) {
			throw new Exception('File not exists: ' . $fullRealPath . '. Path: ' . $path . '. CURRENT_IMPORT_PATH: '
		...
		if (is_dir($fullRealPath)) {
			if (self::$newUpdateMode) {
				...
				$fullScriptPath = realpath($fullRealPath . self::FILE_SEPARATOR . $lastDir . '.php');
				$notChanged = isset(YY::$WORLD->SOURCE->fileTime[$fullScriptPath])
					&& filemtime($fullScriptPath) === YY::$WORLD->SOURCE->fileTime[$fullScriptPath];
				if ($notChanged) {
				...
				$res = null;
				if ($wayOffset === null) {
					$a = explode(self::FILE_SEPARATOR, $path);
	 ...
	 * Есди нечетное, то то же самое, а первый из них содержит имя класса объекта (по умолчанию YY\System\YY\Core\Data).
	 */
	...
	{
		if (substr($path, 0, 1) === self::FILE_SEPARATOR) {
			$path = self::$WORLD_REAL_PATH . $path;
		} else {
			$path = self::$CURRENT_IMPORT_PATH . self::FILE_SEPARATOR . $path;
		}
	...
	{
		$relative = substr($way, 0, 1) !== self::WAY_SEPARATOR;
		if ($relative) {
			$way = self::$CURRENT_IMPORT_WAY . self::WAY_SEPARATOR . $way;
		}
		$way = str_replace('\\', self::WAY_SEPARATOR,
			$way); // TODO: По идее, надо запретить указывать при вызове функции FS обратные слеши, и убрать эту замену
		assert(substr($way, 0, 1) === self::WAY_SEPARATOR);
		$way = substr($way, 1);
		$src = explode(self::WAY_SEPARATOR, $way);
		$dst = [];
		$skip = 0;
		...
		assert(!$skip);
		return self::WAY_SEPARATOR . implode(self::WAY_SEPARATOR, $dst);
	}
	...
	static private function getExistingChild($wayFromRoot, $root)
	{
		...
		if ($wayFromRoot === self::WAY_SEPARATOR) $wayFromRoot = '';
		$props = explode(self::WAY_SEPARATOR, $wayFromRoot);
	...
	{
		$fromWay = explode(self::WAY_SEPARATOR, $fromWay);
		array_shift($fromWay);
		$toWay = explode(self::WAY_SEPARATOR, $toWay);
		array_shift($toWay);
		while (count($fromWay) && count($toWay) && $fromWay[0] === $toWay[0]) {
			array_shift($fromWay);
			array_shift($toWay);
		}
		return str_repeat('..' . self::FILE_SEPARATOR, count($fromWay)) . implode(self::WAY_SEPARATOR, $toWay);
	}
		...
		foreach ($old_keys as $key) {
			...
			if (is_object($val)) {
				if (isset(self::$IMPORT_WAITING[$val])) {
					...
					$val = $newVal;
				} else if ($val instanceof Ref && $val->_OWNER) {
		...
		foreach ($newKeys as $newKey) {
			...
			if (is_object($val) && $val->_OWNER) {
				if (is_object($newKey)) {
					...
					} else if (in_array($newKey, $oldKeys)) { // TODO: Здесь, видимо надо искать не просто, а функцией YY\System\YY\Core\Data::_IsEqual
						$oldKey = $newKey;
					} else $oldKey = null;
				} else {
					$oldKey = $newKey;
				...
				if (isset($old[$oldKey])) {
					...
					if (self::fillObjectMappings($oldChild, $val, $path . self::FILE_SEPARATOR . $newKey)) $hasNewMapping = true;
				}
			}
		}
		...
		foreach ($newKeys as $newKey) {
			...
			if (!is_object($val)) {
				...
				if (!$val->_OWNER) {
					...
					if ($oldChild instanceof Ref) $oldChild = $oldChild->_DAT;
					if ($val instanceof Ref) $val = $val->_DAT;
					if (is_object($oldChild) && get_class($oldChild) === get_class($val)) {
						self::compareObject($oldChild, $val);
						$old[$oldKey] = $oldChild;
					} else {
						$old[$oldKey] = $new->_DROP($newKey); // Не делается копия, так как может быть не YY\System\YY\Core\Data, а производный класс
					}
				} else {
					$old[$oldKey] = $new->_DROP($newKey); // Не делается копия, так как может быть не YY\System\YY\Core\Data, а производный класс
				}
	...
	static private function processWorldPart($wayToPart, $worldTemplateDir, $doLoad)
	{
		...
		if ($originalSubtree instanceof Ref) {
	...
	static public function reloadWorld()
	{
		...
		if (file_exists(CONFIGS_DIR . 'server.id')) { // Признак кластерного устройства
			$serverId = trim(file_get_contents(CONFIGS_DIR . 'server.id'));
	...
	static public function compareWorldPart(
		$wayToPart,
		$worldTemplateDir
	) // TODO: А похоже, эта функция не нужна, а значит и параметр $doLoad в processWorldPart
	{
		...
		if (self::needUpdateNode($currentNode, $sourceFileName)) {
			...
			foreach ($keysArray as $key) {
				...
				if ($val instanceof Ref && $val->_OWNER) {
					...
					if (self::updateNode($wayFromRoot . self::WAY_SEPARATOR . $key, $val, $newValue)) {
						Cache::UpdateData($newValue); // TODO: Может можно убрать? Все равно потом это вызывается для всего дерева.
	...
	static private function importNode($wayFromRoot, $fullSourceScriptPath)
	{
		...
		$sourceInfo->fileNode[$fullSourceScriptPath] = new Ref($res, false);
